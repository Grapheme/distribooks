		...
		if(event.target.readyState == 4){
			if(event.target.status == 200){
				...
				if(response.status == true){
					$("<li></li>").appendTo("ul.resources-images").html(response.responsePhotoSrc);
					$("ul.resources-images").find("a.delete-resource-item:last").off('click').on("click",function(event){
...
function getImagesTotalSize(files){
	var total = 0;
...
if(tests.dnd){
	...
	dropZone.ondrop = function(event){
		...
		uploadDataImage.TotalSize = getImagesTotalSize(files);
		uploadDataImage.CurrentSize = {};
		ReadFiles(files);
	}
}else{
